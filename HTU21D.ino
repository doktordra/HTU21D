...
void handleDebugRoot() {
  ...
  html += "let allChartData=[],chartData=[],cursorIndex=-1,rangeReady=false;const chartDefs=[['temperature','t','#c76432',.6,'C'],['humidity','h','#2878b8',1.5,'%'],['realFeel','r','#8b4aa5',.6,'C'],['absoluteHumidity','a','#087e6a',.6,'g/m3']];function formatMoment(s,full=false){if(s>1700000000){const d=new Date(s*1000);return full?d.toLocaleString():d.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit',second:'2-digit'})}const h=Math.floor(s/3600),m=Math.floor(s%3600/60),q=s%60;return'T+'+[h,m,q].map(n=>String(n).padStart(2,'0')).join(':')}function formatDate(s){return s>1700000000?new Date(s*1000).toLocaleDateString():'od ukljucenja'}function nearestIndex(data,t){let best=0,dist=Infinity;for(let i=0;i<data.length;i++){const d=Math.abs(data[i].s-t);if(d<dist){dist=d;best=i}}return best}";
